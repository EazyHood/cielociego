"""Convierte los articulos de docs/*.md en sus pages HTML.
...
articulo largo, y ahi el diseno tiene que quitarse de en medio -- body en
...
# Un idioma por entrada: fichero fuente, out y los textos de la interfaz.
LANGUAGES = {
    "en": {
        "source": "post.md",
        "target": "post.html",
    ...
    "es": {
        ...
        "desc": ("Sentinel-2 pasa cada cinco días. Sobre dos fields del Caribe colombiano, "
...
def convierte(md: str) -> tuple[str, str]:
    ...
    def cierra() -> None:
        if parrafo:
            body.append("<p>" + en_linea(" ".join(parrafo)) + "</p>")
            parrafo.clear()

    for linea in md.split(NEWLINE):
        s = linea.strip()
        if not s:
            cierra()
        elif s.startswith("# "):
            cierra()
            title = s[2:].strip()
            body.append("<h1>" + en_linea(title) + "</h1>")
        elif s.startswith("## "):
            cierra()
            body.append("<h2>" + en_linea(s[3:].strip()) + "</h2>")
        elif s == "---":
            cierra()
            body.append("<hr>")
        else:
            parrafo.append(s)
    cierra()
    return title, NEWLINE.join(body)


def entradilla(body: str) -> str:
    """La primera parrafada tras el h1 hace de entradilla."""
    partes = body.split("</h1>", 1)
    if len(partes) != 2:
        return body
    resto = partes[1].lstrip(NEWLINE)
    primero, _, cola = resto.partition("</p>")
    return (partes[0] + "</h1>" + NEWLINE
            + primero.replace("<p>", '<p class="entradilla">', 1) + "</p>" + NEWLINE + cola)


def escribe(cfg: dict) -> None:
    source = ROOT / "docs" / cfg["source"]
    target = ROOT / "docs" / cfg["target"]
    title, body = convierte(source.read_text(encoding="utf-8"))
    body = entradilla(body)
    ...
    url = "https://eazyhood.github.io/cielociego/" + cfg["target"]
...
"""
    target.write_text(pagina, encoding="utf-8")
    print("  " + cfg["target"] + ": " + str(target.stat().st_size // 1024)
          + " KB - " + title)
