...
body.push(h2("4.3. Dependencia del tamaño de la parcela, ajustada por el cielo"));
body.push(p("La tabla por tramos muestra la exhaustividad creciendo con el tamaño entre 1 y 100 ha, de 0,429 a 0,520, y cayendo a 0,273 en el tramo de 100 a 500 ha (Tabla 3). Ese último punto corresponde a una sola parcela situada en trópico húmedo, de modo que tamaño y clima quedan confundidos y una línea de tendencia trazada sobre esos puntos no sería evidencia de nada."));
body.push(p("La pregunta se plantea entonces de forma directa sobre el subconjunto que le corresponde: entre las adquisiciones que el filtro descarta, ¿depende del tamaño de la parcela la probabilidad de que estuviera despejada de verdad, manteniendo constante la nubosidad declarada por la tesela? Se ajusta una regresión logística sobre las 2.778 filas rechazadas, de las que 582 (21,0 %) son falsos negativos, con la nubosidad de la tesela y el logaritmo decimal de la superficie como predictores, ambos centrados (Tabla 6)."));
body.push(p("La razón de probabilidades por cada factor diez de superficie es 0,690, con intervalo de confianza del 95 % [0,508, 0,938] y p = 0,018. Manteniendo el cielo constante, cuanto más pequeña es la parcela mayor es la probabilidad de que una adquisición descartada estuviera en realidad despejada sobre ella. Es la dirección que predice el desajuste de soporte, y el intervalo excluye la unidad."));
body.push(p("Sobre el conjunto completo de 3.265 filas legibles, el mismo tipo de modelo indica además que a igualdad de nubosidad declarada una parcela mayor es menos probable que esté enteramente despejada (razón de probabilidades 0,686; IC 95 % [0,509, 0,925]). Geométricamente es lo esperable, puesto que más superficie ofrece más oportunidad de encontrarse una nube, y explica por qué la tabla por tramos, que no ajusta por el cielo, resulta un instrumento demasiado grueso para esta pregunta. El término de interacción entre nubosidad y tamaño no es distinguible de cero (p = 0,15), de modo que no se afirma que la forma de la relación cambie con el tamaño, solo su nivel."));
...
// ---------------------------------------------------------------- figuras
body.push(caption("Tabla 6. Regresión logística sobre las 2.778 adquisiciones que el filtro descarta. Respuesta: la parcela estaba despejada de verdad, es decir, la adquisición era un falso negativo. Predictores centrados. Ajuste por mínimos cuadrados reponderados iterativamente."));
body.push(table(
  ...
  [
    ...
    ["log10 superficie (ha)", "-0,3709", "0,1568", "-2,36", "0,018", "[-0,678, -0,064]"],
  ],
  [2400, 1150, 1050, 950, 1050, 2426],
));
body.push(p("Razón de probabilidades por cada factor diez de superficie: 0,690, IC 95 % [0,508, 0,938]. n = 2.778, de las cuales 582 (21,0 %) son falsos negativos.", { after: 240 }));
