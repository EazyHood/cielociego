...
No calcula nada: solo compone. Si una cifra de aqui no sale de outputs/*.json,
...
R = json.load(open("outputs/summary.json", encoding="utf-8"))
F, C = R["field_fundacion"], R["field_corridor"]
G = {k: open(f"outputs/grafica_{k}.svg", encoding="utf-8").read()
     for k in ("dist", "cal_f", "cal_c", "dur", "anual")}
HUE_F = json.load(open("outputs/field_fundacion_radar.json", encoding="utf-8"))["gaps"]
HUE_C = json.load(open("outputs/field_corridor_radar.json", encoding="utf-8"))["gaps"]
SAR_C = json.load(open("outputs/field_corridor_sar.json", encoding="utf-8"))
SAR_F = json.load(open("outputs/field_fundacion_sar.json", encoding="utf-8"))
ANA_C = SAR_C.get("analysis", {})
ANA_F = SAR_F.get("analysis", {})
...
VECES = abs((TEND_C.get("slope") or 1) / (TEND_FU.get("slope") or 1))
NEWLINE = (FORMA_C.get("level_after") or 0) - (FORMA_C.get("level_before") or 0)
DBIC = FORMA_C2.get("bic", 0) - FORMA_C.get("bic", 0)

for _k in ("sar_c", "sar_f", "ctrl"):
    G[_k] = open(f"outputs/grafica_{_k}.svg", encoding="utf-8").read()


def _tendencia(medidas, solo=None):
    m = sorted(medidas, key=lambda x: x["date"])
    if solo:
        m = [x for x in m if x["platform"].lower() == solo]
    f = np.array([date.fromisoformat(x["date"]).toordinal() for x in m], dtype=float)
    v = np.array([x["vv_db"] for x in m])
    return float(np.polyfit(f, v, 1)[0] * 365), len(m)


TEND_TODAS, N_TODAS = _tendencia(SAR_C["measured"])
TEND_S1A, N_S1A = _tendencia(SAR_C["measured"], "sentinel-1a")
TEND_FUND, N_FUND = _tendencia(SAR_F["measured"], "sentinel-1a")
_vv = np.array([x["vv_db"] for x in sorted(SAR_C["measured"], key=lambda y: y["date"])])
SUBIDA = float(_vv[-20:].mean() - _vv[:20].mean())

TOT_LARGOS = F["long_gaps"] + C["long_gaps"]
TOT_CUB = F["long_covered"] + C["long_covered"]
TOT_HUECOS = len(HUE_F) + len(HUE_C)
FN = F["false_neg"] + C["false_neg"]
FP = F["false_pos"] + C["false_pos"]
TOMAS = F["measured"] + C["measured"]
DIAS = 2794
...
.record{display:grid; grid-template-columns:repeat(auto-fit,minmax(148px,1fr));
  gap:1px; background:var(--linea); border:1px solid var(--linea)}
.record>div{background:var(--superficie); padding:1.05rem 1.15rem;
  display:flex; flex-direction:column; gap:.32rem}
.record .cifra{font-family:"IBM Plex Mono",monospace; font-size:1.7rem; font-weight:500;
  line-height:1; letter-spacing:-.02em; font-variant-numeric:tabular-nums}
.record .pie{font-size:.78rem; line-height:1.38; color:var(--tinta-suave)}
...
.paso>.body{display:flex; flex-direction:column; gap:1.3rem; min-width:0}
...
.warning{background:var(--superficie); border:1px solid var(--linea);
...
def fila(name, d):
    pct = 100 * d["usable"] / d["measured"]
    return (
        f'<tr><td>{name}</td>'
        f'<td class="num">{n(d["measured"])}</td>'
        f'<td class="num">{n(d["usable"])}</td>'
        f'<td class="num">{n(pct)} %</td>'
        f'<td class="num">{n(d["usable_by_tile"])}</td>'
        f'<td class="num" style="color:var(--ciego)">{n(d["false_neg"])}</td>'
        f'<td class="num">{n(d["false_pos"])}</td></tr>'
    )
...
<header class="pila g3">
  ...
  dos fields, cuánto alcanzaba a ver el satélite óptico. La respuesta: <b>el {n(F["pct_blind"])} %
  y el {n(C["pct_blind"])} % de los días no hubo una sola observación aprovechable.</b> El radar,
...
<section class="record">
  ...
  <div><span class="cifra">{n(len(SAR_C["measured"]) + len(SAR_F["measured"]))}</span><span class="pie">medidas de retrodispersión de radar sobre el polígono</span></div>
...
2020-01-09   linea N0500  ->  nube  0,05 %
    ...
    <p class="nota medida">Deduplicar por date no las junta, porque el sensado difiere en un
...
<article class="paso">
  <div class="num">2</div>
  <div class="body">
    <h2>El dato de nube que todo el mundo usa no usable para un field</h2>
    <div class="pila g2 medida">
      <p>Cada escena trae un campo de nubosidad, y es el que se usa para decidir si una imagen vale
      la pena. Está calculado sobre la tile entera: <b>110 × 110 km, 12.100 km²</b>. El field de
      ...
      kilómetro, así que un field pequeño <b>o está debajo de ella o no lo está</b>:</p>
    </div>
    <figure>
      <div class="lienzo">{G["dist"]}</div>
      <figcaption>Fundación, {n(F["measured"])} scenes. En el field, el {n(100 * F["extremes_field"])} %
      de las scenes cae en un extremo — despejado del todo o tapado del todo. En la tile, solo el
      {n(100 * F["extremes_tile"])} %.</figcaption>
    </figure>
    <div class="balance">
      <div class="err"><span class="cifra">{n(FN)}</span>
        <span class="rotulo">veces que la tile dijo <b>inservible</b> y el field se veía
        perfectamente</span></div>
      <div class="ok"><span class="cifra">{n(FP)}</span>
        <span class="rotulo">veces que dijo <b>servible</b> y el field estaba tapado</span></div>
    </div>
    <p class="nota medida">Una asimetría de <b>37 a 1</b>. Filtrar por el número de la tile no es
    ...
    <div class="envoltorio"><table>
      <thead><tr>
        <th>Field</th><th>Tomas</th><th>Útiles reales</th><th>%</th>
        <th>Útiles según tile</th><th>Buenas descartadas</th><th>Malas coladas</th>
      </tr></thead>
...
<article class="paso">
  <div class="num">3</div>
  <div class="body">
    <h2>Con la medida honesta, el field pasa casi todo el año invisible</h2>
    <div class="pila g2 medida">
      <p>Contando como útil toda toma con menos del 10 % del field tapado — un criterio generoso —
      quedan <b>{n(F["usable"])} días aprovechables de {n(DIAS)}</b> en Fundación y {n(C["usable"])}
    ...
    <div class="warning pila g2">
      <h3>El peor tramo del corredor duró {C["worst"]["days"]} días</h3>
      <p class="suave">Del <span class="mono">{C["worst"]["start"]}</span> al
      <span class="mono">{C["worst"]["end"]}</span> no hubo una sola imagen óptica utilizable del
      field. Casi tres meses seguidos. En Fundación el peor fue de {F["worst"]["days"]} días, del
      {F["worst"]["start"]} al {F["worst"]["end"]}. Un ciclo de banano no backoff tres meses a que
      escampe.</p>
    </div>
  </div>
</article>
...
<article class="paso">
  ...
  <div class="body">
    ...
    <div class="pila g2 medida">
      ...
      retrodispersión sobre el polígono — <b>{n(len(SAR_C["measured"]) + len(SAR_F["measured"]))}
    ...
    <pre>nivel estable hasta   {FORMA_C.get("cut", "?")}      {n(FORMA_C.get("level_before") or 0, 2)} dB
TRANSICION            {FORMA_C.get("cut", "?")} -> {FORMA_C.get("cut_end", "?")}
nivel estable desde   {FORMA_C.get("cut_end", "?")}      {n(FORMA_C.get("level_after") or 0, 2)} dB
                                           <span class="suave">salto {n(NEWLINE, 2)} dB</span></pre>
    <figure>
      <div class="lienzo">{G["sar_c"]}</div>
      <figcaption>Corredor bananero, órbita 77, {n(N_TODAS)} pasadas. Cada punto es la media del
      field en γ⁰ VV; la línea es <b>el model que gana</b>, no una recta impuesta. Las franjas
    ...
    <figure>
      ...
      <figcaption>No desaparece: dentro de Sentinel-1A sola la slope es {n(TEND_S1A, 3)} dB/año,
      incluso mayor que mezclando plataformas. En el mismo año, S1A y S1B difieren entre 0,04 y
      0,50 dB: no hay sesgo de platform que explique 3,5.</figcaption>
    </figure>
    <div class="balance">
      <div class="err"><span class="cifra">{n(TEND_C.get("slope", 0), 2)}</span>
      ...
      <div class="ok"><span class="cifra">{n(VECES, 0)}×</span>
        <span class="rotulo">menos cambia el field vecino: {n(TEND_FU.get("slope", 0), 3)} dB/año,
        y en sentido contrario</span></div>
    </div>
    <p class="nota medida"><b>Y aquí hay que hilar fino: el vecino no es exactamente «plano».</b>
    En la serie completa su slope es {n(TEND_FU.get("slope", 0), 3)} dB/año con
    IC 95 % [{n(_IC_F[0], 3)}, {n(_IC_F[1], 3)}] — pequeña, pero el intervalo no cross el cero.
    Restringido a Sentinel-1A, el mismo field da −0,022 con IC [−0,093, +0,049], que <em>sí</em>
    lo cross: <b>con ese subconjunto es indistinguible de plano; con la serie entera hay un
    ...
    <div class="pila g2 medida">
      ...
      los residuos arrastran ({n(TEND_C.get("residual_autocorr", 0), 2)} de autocorrelación), así
      que las <b>{n(TEND_C.get("n", 0))} pasadas valen como
      {n(TEND_C.get("n_effective", 0), 0)} observaciones independientes</b>. El error estándar
      clásico se quedaba <b>{n(TEND_C.get("inflation_se", 1), 1)} veces corto</b>; el que se publica
      aquí es el corregido.</p>
      <p class="nota"><b>Y no depende de un año suelto.</b> Recalculando la slope y quitando
    ...
    <figure>
      ...
      <figcaption>Fundación, órbita 77. El mismo procedimiento sobre el field vecino.</figcaption>
    </figure>
    <div class="warning pila g2">
      <h3>Y hay una corroboración independiente</h3>
      <p class="suave">Sobre este mismo field se había hecho antes un análisis <b>óptico</b>, sin
      ...
      <p class="suave">El radar, por su cuenta, date la transición entre <b>jun-2021 y ago-2023</b>.
...
<section class="pila g3">
  ...
  <div class="pila g2 medida">
    ...
    conclusión.</p>
    <p class="nota"><b>La máscara de nubes es un model, y los models se actualizan.</b> El
    ...
    despejado y la otra lo daba <b>71,8 % tapado</b>, sobre la misma toma. <b>El 6,6 % de las scenes
    cross el umbral de utilidad</b>, y siempre en el mismo sentido: la versión nueva marca más
...
<footer class="pila g2">
  <p><b>¿Primera vez aquí?</b> Hay una versión explicada, en inglés y sin dar nada por sabido:
  <a href="post.html">How often can a satellite actually see your field?</a></p>
  ...
  <p>Citable con DOI: <a href="https://doi.org/10.5281/zenodo.22132250">10.5281/zenodo.22132250</a> (Zenodo, acceso abierto).</p>
  <p>© 2026 Jhonatan del Rio Mejia. Todos los derechos reservados. El texto y las gráficas de
  esta página están bajo <a href="https://creativecommons.org/licenses/by-nc-nd/4.0/deed.es">CC BY-NC-ND 4.0</a>;
  el código, bajo MIT. Las mediciones no son de nadie: reprodúcelas.</p>
  <p class="mono" style="font-size:.79rem">cielociego v0.1.1 · medido el 26 de agosto de 2026 ·
...
with open("outputs/report.html", "w", encoding="utf-8") as fh:
    fh.write(HTML)
